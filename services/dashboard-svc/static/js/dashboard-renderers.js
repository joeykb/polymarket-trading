...
function renderAlertsFeed(alertsArr) {
    ...
    for (let i = 0; i < Math.min(groups.length, maxShow); i++) {
        ...
        const countBadge =
            g.count > 1
                ? '<span style="background:rgba(251,191,36,0.2);color:#fbbf24;font-size:10px;font-weight:700;padding:1px 6px;border-radius:99px;margin-left:6px;">\u00d7' +
                  g.count +
                  '</span>'
                : '';
        const rangeStr =
            g.count > 1 && g.firstTimestamp
                ? '<span style="color:var(--text-muted);font-size:10px;margin-left:4px;">(since ' +
                  formatAlertTime(g.firstTimestamp) +
                  ')</span>'
                : '';
        h +=
            '<div class="alert-item"><span class="alert-icon">' +
            icon +
            '</span><div class="alert-content"><div class="alert-message">' +
            escapeHtml(g.message) +
            countBadge +
            '</div><div class="alert-time">' +
            timeStr +
            rangeStr +
            '</div></div></div>';
    }
    if (groups.length > maxShow) {
        h +=
            '<div class="alert-item" style="justify-content:center;color:var(--text-muted);font-size:12px;">+ ' +
...
function formatAlertTime(iso) {
    try {
        ...
        if (sameDay) {
            return (
                'Today ' + d.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true, timeZone: 'America/New_York' })
            );
        }
        return (
...
function renderRangesTable(ranges, target, below, above) {
    ...
    let html =
        '<table class="ranges-table"><thead><tr><th></th><th>Range</th><th>YES Price</th><th>Implied %</th><th>Volume</th><th>Probability</th></tr></thead><tbody>';
    for (const r of sorted) {
        let rowClass = '',
            marker = '';
        if (target && r.marketId === target.marketId) {
            rowClass = 'selected-target';
            marker = '🎯';
        } else if (below && r.marketId === below.marketId) {
            rowClass = 'selected-below';
            marker = '⬇️';
        } else if (above && r.marketId === above.marketId) {
            rowClass = 'selected-above';
            marker = '⬆️';
        }
        ...
        html +=
            '<td style="min-width:120px;"><div class="price-bar"><div class="price-bar-fill" style="width:' +
            Math.min(pct, 100) +
            '%;background:' +
            barColor +
            ';"></div></div></td></tr>';
...
function renderChart(snapshots) {
    ...
    for (let i = 0; i <= 5; i++) {
        ...
        yLabels +=
            ...
            '" y="' +
            (y + 4) +
            '" fill="var(--text-muted)" font-size="11" text-anchor="end" font-family="JetBrains Mono">' +
            val.toFixed(1) +
            '¢</text>';
        yLabels +=
            '<line x1="' +
    ...
    for (let i = 0; i < xLabelCount; i++) {
        ...
        xLabels +=
            ...
            '" y="' +
            (height - 8) +
            '" fill="var(--text-muted)" font-size="10" text-anchor="middle" font-family="JetBrains Mono">' +
            formatTime(snapshots[idx].timestamp) +
            '</text>';
    }
    const lY = pad.top - 2;
    let legend =
        ...
        '" y="' +
        (lY - 8) +
        '" width="10" height="10" rx="2" fill="var(--accent-blue)" /><text x="' +
        (pad.left + 14) +
        '" y="' +
        lY +
        '" fill="var(--text-secondary)" font-size="11">Target</text>';
    legend +=
        '<rect x="' +
        (pad.left + 70) +
        '" y="' +
        (lY - 8) +
        '" width="10" height="10" rx="2" fill="var(--accent-orange)" /><text x="' +
        (pad.left + 84) +
        '" y="' +
        lY +
        '" fill="var(--text-secondary)" font-size="11">Below</text>';
    legend +=
        '<rect x="' +
        (pad.left + 134) +
        '" y="' +
        (lY - 8) +
        '" width="10" height="10" rx="2" fill="var(--accent-green)" /><text x="' +
        (pad.left + 148) +
        '" y="' +
        lY +
        '" fill="var(--text-secondary)" font-size="11">Above</text>';
    return (
        '<svg viewBox="0 0 ' +
        width +
        ' ' +
        height +
        '" class="chart-canvas" preserveAspectRatio="xMidYMid meet">' +
        yLabels +
        xLabels +
        legend +
...
async function retryPosition(positionId, btnEl) {
    ...
    try {
        const res = await fetch('/api/retry-position', {
            method: 'POST',
...
async function sellPosition(posDataStr, btnEl) {
    ...
    if (typeof posDataStr === 'number') {
        ...
        try {
            posData = JSON.parse(posDataStr);
        } catch {
            posData = { positionId: parseInt(posDataStr) || posDataStr };
        }
    ...
    if (isNaN(sharesToSell) || sharesToSell <= 0) {
        alert('Invalid share count');
        return;
    }
    if (sharesToSell > totalShares) {
        alert('Cannot sell more than ' + totalShares + ' shares');
        return;
    }
    ...
    btnEl.disabled = true;
    btnEl.textContent = 'Selling ' + sharesToSell + '...';
    try {
        const payload = {
...
document.addEventListener('click', function (e) {
    ...
    if (!btn) return;
    const raw = btn.getAttribute('data-sell');
    if (!raw) return;
    e.preventDefault();
    e.stopPropagation();
