...
async function incrementalUpdate(data) {
    ...
    updateStatCard(
        'stat-current',
        vm.currentTempF !== null ? vm.currentTempF + '\u00b0F' : '--',
...
async function render(data) {
    ...
    if (!vm.session && !vm.observation) {
        ...
        let noDataHtml =
            ...
            '</div>';
        noDataHtml +=
            '<div class="no-data"><h2>No monitoring data for ' +
            currentDate +
            '</h2><p>Start the monitor with:<br><code>node src/monitor.js</code></p></div><div id="configPanel"></div>';
    ...
    let html =
        ...
        statCard(
            ...
            currentConditions + (maxTodayF ? ' \u00b7 Hi: ' + maxTodayF + '\u00b0F' : ''),
            0,
            '',
        ) +
        statCard(
            ...
            'Forecast High',
            forecastTemp + '\u00b0F',
            sourceLabel + (daysUntilTarget !== null ? ' \u00b7 T-' + daysUntilTarget : ''),
            forecastChange,
            '\u00b0F',
        ) +
        statCard(
            'stat-target',
            'Target Range',
            shortLabel(targetRange?.question || '--'),
            session?.initialTargetRange ? 'Initial: ' + shortLabel(session.initialTargetRange) : '',
            0,
            '',
        ) +
    ...
    if (session?.resolution) {
        ...
        html +=
            '<div class="card" style="border-color:var(--accent-green);margin-bottom:24px;"><div class="card-header" style="background:rgba(16,185,129,0.1);"><span class="card-title">🎯 Resolve Day — Range Decision</span></div><div class="card-body"><div style="font-size:18px;font-weight:700;color:var(--accent-green);margin-bottom:8px;">KEEP: ' +
            escapeHtml(shortLabel(r.keep)) +
            ' (' +
            (r.keepPrice * 100).toFixed(1) +
            '¢)</div><div style="color:var(--accent-red);margin-bottom:8px;">DISCARD: ' +
            r.discard.map((d) => escapeHtml(shortLabel(d))).join(', ') +
            '</div><div style="color:var(--text-secondary);font-size:13px;">' +
            escapeHtml(r.reason) +
            '</div></div></div>';
    }
    html +=
        '<div class="grid-2col"><div class="card"><div class="card-header"><span class="card-title">📈 Price History</span><span id="snapshotCountLabel" style="font-size:12px;color:var(--text-muted);">' +
        snapshotCount +
        ' snapshots</span></div><div class="card-body"><div class="chart-container" id="chartContainer">' +
        (snapshots.length >= 2 ? renderChart(snapshots) : '<div class="chart-empty">Waiting for more snapshots to plot chart...</div>') +
        '</div></div></div>';
    html +=
        '<div class="card"><div class="card-header"><span class="card-title">🔔 Alerts</span><span id="alertCountLabel" style="font-size:12px;color:var(--text-muted);">' +
        alertCount +
        ' total</span></div><div class="card-body" style="padding:0;max-height:280px;overflow-y:auto;" id="alertsBody">' +
        renderAlertsFeed(alerts) +
        '</div></div></div>';
    html +=
        '<div class="card" id="liquidityCard" style="display:none;"><div class="card-header"><span class="card-title">📡 Live Liquidity</span><span id="liquidityStatus" style="font-size:11px;color:var(--text-muted);font-family:JetBrains Mono,monospace;">connecting...</span></div><div class="card-body" id="liquidityBody" style="padding:0;"><div style="padding:24px;text-align:center;color:var(--text-muted);font-size:13px;">Connecting to order book stream...</div></div></div>';
    html +=
        '<div class="card" id="pipelineCard" style="display:none;"><div class="card-header"><span class="card-title">🔭 Forecast Pipeline</span><span id="pipelineCount" style="font-size:12px;color:var(--text-muted);">loading...</span></div><div class="card-body" id="pipelineBody" style="padding:0;"><div style="padding:24px;text-align:center;color:var(--text-muted);font-size:13px;">Loading pipeline data...</div></div></div>';
    html +=
    ...
    html +=
        '<div class="card"><div class="card-header"><span class="card-title">📊 All Temperature Ranges</span></div><div class="card-body" style="padding:0;overflow-x:auto;" id="rangesTableBody">' +
        renderRangesTable(allRanges, targetRange, belowRange, aboveRange) +
        '</div></div>';
