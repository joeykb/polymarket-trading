...
> **⚠️ CRITICAL: Standard and neg-risk markets use DIFFERENT redemption contracts and parameters! See [redemption.md](./redemption.md) for complete details.**
...
Call `NegRiskAdapter.redeemPositions(conditionId, amounts[])`:

| Parameter | Type | Value |
|-----------|------|-------|
| `conditionId` | bytes32 | Market's condition ID |
| `amounts` | uint256[] | `[yesTokenAmount, noTokenAmount]` — **actual amounts, NOT indexSets!** |

> **The `amounts` parameter is the actual token quantity (e.g., `[5000000, 0]` for 5.0 shares), NOT indexSets like `[1, 2]`. Passing `[1, 2]` will produce a near-zero payout.**

Requires CTF approval: `ctf.setApprovalForAll(NEG_RISK_ADAPTER, true)`.
Must be called from the wallet holding the tokens (EOA, not Safe/relayer).
...
3. `getPositionId(collateralToken, collectionId)` — collateralToken = **USDC.e**

### Neg-Risk Markets

1. `getConditionId(oracle, questionId, outcomeSlotCount)` — oracle = **NegRiskAdapter** (not UMA!), outcomeSlotCount = 2
2. `getCollectionId(parentCollectionId, conditionId, indexSet)` — parentCollectionId = bytes32(0), indexSet = 1 (Yes) or 2 (No)
3. `getPositionId(collateralToken, collectionId)` — collateralToken = **wcol** (`0x3A3BD7bb9528E159577F7C2e685CC81A765002E2`, not USDC.e!)

The NegRiskAdapter has a helper: `adapter.getPositionId(questionId, true/false)` which computes this automatically.
